<footer>
  <div class="footer">
    <div class="footer-top">
      <div>
        <b class="footer-header">TastyTasty</b>
        <ul>
          <li>
            <a href="${pageContext.request.contextPath}/menu">View Menu</a>
          </li>
        </ul>
      </div>
      <div>
        <b class="footer-header">Information</b>
        <ul>
          <li>
            <a href="${pageContext.request.contextPath}/admin/login">Administrator</a>
          </li>
          <li>
            <a href="${pageContext.request.contextPath}/about">About Us</a>
          </li>
        </ul>
      </div>
      <div>
        <b class="footer-header">Follow us on</b>
        <ul>
          <li>
            <a href="https://github.com/datwaft/proyecto-restaurantes" title="Github">
              <i class="fab fa-github"></i>
            </a>
          </li>
        </ul>
      </div>
    </div>
    <div class="footer-bot">
      <p>©2020 TastyTasty - Made by David Guevara & Mario Arguello</p>
    </div>
  </div>
</footer>
